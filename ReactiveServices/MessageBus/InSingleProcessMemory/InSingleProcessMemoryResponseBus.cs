...
{
    public class InSingleProcessMemoryResponseBus : InSingleProcessMemoryMessageBus, IResponseBus
    {
        private static readonly Logger Log = LogManager.GetCurrentClassLogger();

        private readonly Dictionary<Tuple<Type, SubscriptionId>, Tuple<InSingleProcessMemorySubscription, CancellationTokenSource>> ActiveSubscriptions = new Dictionary<Tuple<Type, SubscriptionId>, Tuple<InSingleProcessMemorySubscription, CancellationTokenSource>>();
            ...
            TimeSpan expiration)
        {
            ...
            Task.Run(() =>
            {
                while (!respondingTaskCancellationTokenSource.IsCancellationRequested) // Blocks until a request is received
                {
                    if (request != null)
                    {
                        //Note: Expired requests will be handled, and ignored, by InSingleProcessMemory itself
            ...
            ActiveSubscriptions[key] = new Tuple<InSingleProcessMemorySubscription, CancellationTokenSource>(subscription, respondingTaskCancellationTokenSource);
        }

        protected virtual InSingleProcessMemorySubscription NewRequestSubscription(Type requestType, string requestQueueName, Action<object, object> messageHandler)
        {
            var subscription = new InSingleProcessMemorySubscription(this, null, TopicId.None, requestType, requestQueueName, SubscriptionMode.Shared, true, null, messageHandler, null, null);
            return subscription;
        }
            ...
            TimeSpan expiration)
        {
            ...
            {
                Log.Error(String.Format("Could not publish response of type '{0}'", responseType.Name), e);
                throw;
            }
        }
        ...
        public void StopRespondingTo(Type requestType, SubscriptionId subscriptionId = null)
        {
            ...
            Tuple<InSingleProcessMemorySubscription, CancellationTokenSource> subscriptionAndCancellationTokenSource;
